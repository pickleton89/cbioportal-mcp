...
class Configuration:
    ...
    def _validate_transport(self):
        ...
        if transport not in valid_transports:
            raise ConfigurationError(
                f"server.transport must be one of: {valid_transports}"
            )
    ...
    def _validate_numeric_ranges(self):
        ...
        if not isinstance(rps, (int, float)) or rps <= 0:
            raise ConfigurationError(
                "api.rate_limit.requests_per_second must be positive"
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "create-example":
        output_file = (
            sys.argv[2] if len(sys.argv) > 2 else "cbioportal-mcp-config.example.yaml"
        )
