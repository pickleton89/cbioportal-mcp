...
async def test_sequential_vs_concurrent():
    ...
    study_ids = ["acc_tcga", "blca_tcga", "brca_tcga", "cesc_tcga", "chol_tcga", 
...
async def test_gene_batch_concurrency():
    ...
    gene_ids = ["TP53", "KRAS", "BRCA1", "BRCA2", "EGFR", "PTEN", "RB1", 
               "AKT1", "PIK3CA", "VEGFA", "MYC", "ERBB2", "CDK4", "CDKN2A"]
    ...
    for gene_id, gene_data in list(result["genes"].items())[:3]:  # Just show first 3
        print(f"{gene_id}: {gene_data.get('hugoGeneSymbol')} (Entrez ID: {gene_data.get('entrezGeneId')})")
