...
async def test_get_study_details(
    mock_api_request, cbioportal_server_instance, mock_study_data
):
    ...
    [
        ...
        (
            ...
            httpx.RequestError(
                ...
                request=httpx.Request(
                    "GET", "/api/genes/fetch"
                ),  # Endpoint for get_genes is /genes/fetch (POST)
            ),
...
async def test_generic_api_error_handling(
    ...
    if (
        ...
        actual_args = list(
            method_args[0]
        )  # Unpack the inner tuple for get_genes's gene_ids list
    else:
...
async def test_get_multiple_studies_partial_failure(
    mock_make_api_request, cbioportal_server_instance, mock_study_detail_brca
):
    server = cbioportal_server_instance
